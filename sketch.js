...
function ticket() {
  ...
  if(mouseX>272&&mouseX<303&&mouseY<437&&mouseY>406){
     ...
     strokeWeight(1);
     ellipse(285, 420, 10);
     line(275, 420, 200, 420);
     fill(255, 70);
     stroke(0);
     rectMode(CENTER);
     rect(100, 420, 200, 240);
     textAlign(CENTER);
     textSize(10);
     textFont(font2);
     text("당신이 세상에 대한 느끼는 거리감에 따라 주사위 안 숫자가 달라집니다.", 100, 320);
     text("당신에게 세상이 친밀하고 충만하게 다가오는지, 아니면 약간은 낯설거나 거리감이 느껴지는지에 따라 말이지요.", 100, 385);
     text("주사위의 숫자가 작아질수록 당신은 세상에 담담히 자리잡은 돌벽과 같을 것이고,", 100, 450);
     text("주사위의 숫자가 커질수록 세상을 기꺼이 끌어안으려는 품과 같은 사람일 것입니다.", 100, 520);
  }
}
