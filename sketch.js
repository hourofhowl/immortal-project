...
function ticket() {
  ...
  textAlign(CENTER);
  textFont(font2);
  text("티켓의 요소들에 마우스 커서를 올려보세요", 975, 160);
  ...
  if (mouseX > 272 && mouseX < 303 && mouseY < 437 && mouseY > 406) {
    ...
    strokeWeight(1);
    ellipse(480, 400, 50);
    line(480, 425, 480, 515);
    fill(0, 150);
    stroke(0);
    rectMode(CENTER);
    rect(480, 565, 340, 100);
    rect(860, 565, 380, 100);
    textAlign(CENTER);
    textSize(15);
    textFont(font2);
    fill(255);
    stroke(255);
    text("켜켜이 쌓여 당신의 내면을 이루고 있을\n감정들이 궁금합니다.\n당신이 사랑하는 계절의 빛깔에,\n미묘한 감정의 조도가 덧입혀져\n티켓의 색으로 표현되었습니다.", 480, 565);
    if (countA_2 >= maxQ2) {
      text("향기를 느끼는 방식에 따라\n티켓의 질감이 표현됩니다.\n잘 다듬어진 조약돌처럼,\n당신은 매끈하고 넉살좋게 향기를 받아들이는 것 같습니다.", 860, 565);
    } else if (countB_2 >= maxQ2) {
      text("향기를 느끼는 방식에 따라\n티켓의 질감이 표현됩니다.\n당신은 까끌거리는 모래처럼,\n세밀하고 민감하게 향기를 받아들이는 것 같습니다.", 860, 565);
    } else if (countC_2 >= maxQ2) {
      text("향기를 느끼는 방식에 따라\n티켓의 질감이 표현됩니다.\n당신은 거친 나무처럼\n돌풍에도 묵묵히 뿌리 박힌 채, \n향기를 깊이 들이마시는 것 같습니다.", 860, 565);
    } else if (countD_2 >= maxQ2) {
      text("향기를 느끼는 방식에 따라\n티켓의 질감이 표현됩니다.\n당신은 흐르는 물결처럼\n자연스럽고 풍성하게 향기를 받아들이는 것 같습니다.", 860, 565);
    } else if (countE_2 >= maxQ2) {
      text("향기를 느끼는 방식에 따라\n티켓의 질감이 표현됩니다.\n당신은 제각기 반짝이는 별들처럼,\n개성넘치게 향기를 받아들이는 것 같습니다.", 860, 565);
    }
